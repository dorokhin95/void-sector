...
gfx.buildSettings=function(){
 ...
 const render=()=>{
  ...
  const audioRows=globalThis.audioAPI?['master','music','sfx'].map(k=>'<label>'+({master:'ОБЩАЯ ГРОМКОСТЬ',music:'МУЗЫКА',sfx:'ЭФФЕКТЫ'}[k])+'<input type="range" min="0" max="1" step=".02" data-vol="'+k+'" value="'+audioAPI.getVolume(k)+'"></label>').join(''):'<p class="hint">Звуковая система не загружена.</p>';
  const voice=globalThis.storyAPI?'<label>СИСТЕМНЫЙ ГОЛОС ДИАЛОГОВ (если в системе есть русский голос)<button class="toggle" id="voiceToggle" aria-pressed="'+(storyAPI.getVoice?.()?'true':'false')+'">'+(storyAPI.getVoice?.()?'ВКЛ':'ВЫКЛ')+'</button></label>':'';
