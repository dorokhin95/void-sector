...
function spaceBuildDecor(theme,rnd){
 ...
 // Mobile-only: примерно вдвое меньше декора при том же уровне качества, чем PC
 // (см. Performance budget background в задании — Mobile HIGH secondary 2-5 / far 4-8
 // против PC HIGH 4-8 / 8-16). Это единственное отличие от VOID-SECTOR/gfx-space.js.
 const MOBILE_DECOR_SCALE=.55;
 const budget=Math.max(.3,(gfx.preset?.particles??1)*MOBILE_DECOR_SCALE);
