...
const audioAPI=(function(){
 ...
 // ---------- Озвучка диалогов: предзаписанные реплики через LRU-кэш AudioBuffer'ов ----------
 // Тот же код, что в мобильной версии (единый voice pack для PC и Mobile). Один AudioContext,
 // декодированные буферы проигрываются через voiceBus; MAX_DECODED_VOICE_BUFFERS ограничивает
 // память — весь пак разом никогда не декодируется, только реально прозвучавшие реплики.
 // iOS/Telegram-специфичное восстановление контекста сюда не переносилось — оно не нужно на PC.
 ...
 const api={
  ...
  play:playSfx,duck,unlock,preloadVoice,playVoice,stopVoice,
